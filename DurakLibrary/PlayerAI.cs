...
namespace DurakLibrary
{
    ...
    {
        //Метод "AttackingPhase", который определяет, какая карта должна быть сыграна во время фазы атаки компьютерного игрока. Метод получает объект "GameRiver",
        //который представляет игровую реку (коллекцию карт), на которую игрок будет играть свою карту. В методе выполняется переключение по длине игровой реки, и в
        //каждом кейсе определяется, какую карту нужно сыграть, чтобы продолжить атаку. Если игрок не может сыграть карту, он переходит в фазу защиты.
        //Метод "DefendingPhase", который определяет, какая карта должна быть сыграна во время фазы защиты компьютерного игрока.Метод получает объект "GameRiver" и 
        //    козырную карту.В методе выполняется переключение по длине игровой реки, и в каждом кейсе определяется, какую карту нужно сыграть, чтобы защититься от предыдущей атаки.
        public PlayerAI()
        {
